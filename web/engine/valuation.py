...
# the 19 MB advanced-stats table is read only when production_scores.json is
# absent; the browser build ships the json and never loads the table
A = None
...
def production_scores(seasons=(2024, 2025, 2026)):
    global A
    if os.path.exists(_p('production_scores.json')):
        import json
        return json.load(open(_p('production_scores.json')))
    if A is None:
