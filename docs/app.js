...
function developmentPanel(pid, reload) {
  ...
  box.append(el('div', { class: 'tiles', style: 'grid-template-columns:repeat(4,1fr);margin-bottom:12px' },
    ...
    el('div', { class: 'tile' }, el('div', { class: 'h5' }, 'Raise the Ceiling'), el('div', { class: 'word', style: 'font-size:18px' }, d.unlock_cost != null ? `${d.unlock_cost.toLocaleString()} XP` : '—'), el('div', { class: 'sub' }, el('button', { class: 'btn' + (d.unlock_ok ? ' go' : ''), disabled: d.unlock_ok ? null : '', style: 'padding:3px 10px;font-size:13px;margin-top:4px', 'data-tip': 'One more point of overall he can grow into', onclick: () => act('unlock_ceiling') }, 'Unlock +1'))),
    el('div', { class: 'tile' }, el('div', { class: 'h5' }, 'Auto-Spend'), el('div', { class: 'word', style: 'font-size:18px' }, d.auto ? 'On' : 'Off'), el('div', { class: 'sub' }, el('button', { class: 'btn', style: 'padding:3px 10px;font-size:13px;margin-top:4px', 'data-tip': "The assistants spend his XP each week by the same read the league's clubs use", onclick: () => act('auto_xp', `on=${d.auto ? 'False' : 'True'}`) }, d.auto ? 'Turn Off' : 'Turn On'), ' ', el('button', { class: 'btn quiet', style: 'padding:3px 10px;font-size:13px;margin-top:4px', 'data-tip': 'Spend what he has now, once, by the assistants\' read', onclick: () => act('spend_by_read') }, 'Spend by Read')))));
  const t = el('table', { class: 'tbl' }); t.append(el('tr', {}, el('th', {}, 'Attribute'), el('th', { class: 'n' }, 'Now'), el('th', { class: 'n', 'data-tip': 'How much of his overall this attribute carries at his position' }, 'Weight'), el('th', { class: 'n', 'data-tip': 'Overall gained by the next point' }, '+Ovr'), el('th', { class: 'n', 'data-tip': 'XP for the next point; the price rises with every point bought, with age, and 2.5x for a physical' }, 'Next Point'), el('th', {}, '')));
...
function renderProgression(v) {
  ...
  s.append(el('div', { class: 'tools' }, el('button', { class: 'btn' + (v.auto_all ? ' go' : ''), 'data-tip': "Every player's XP spent each week by the assistants' read, the way the league's clubs do it", onclick: () => { notify(pyJSON(`SESSION.club_act('auto_xp', on=${v.auto_all ? 'False' : 'True'})`)); reload(); } }, v.auto_all ? 'Auto-Spend: On for All' : 'Turn Auto-Spend On for All'),
    el('button', { class: 'btn', 'data-tip': 'Spend every bank now, once, by the read', onclick: () => { notify(pyJSON(`SESSION.club_act('spend_by_read')`)); reload(); } }, 'Spend All by Read'),
    el('span', { class: 'count', style: 'margin-left:auto' }, 'Open a player for his Development tab')));
  const t = el('table', { class: 'tbl' }); t.append(el('tr', {}, el('th', {}, 'Player'), el('th', {}, 'Pos'), el('th', { class: 'n' }, 'Age'), el('th', { class: 'n' }, 'Ovr'), el('th', { class: 'n' }, 'Ceiling'), el('th', { class: 'n', 'data-tip': 'Overall he can still grow into' }, 'Room'), el('th', { class: 'n' }, 'XP Banked'), el('th', { class: 'n', 'data-tip': 'The cheapest next point' }, 'Next Point'), el('th', { class: 'n' }, 'Bought This Year'), el('th', {}, 'Auto'), el('th', {}, '')));
...
function renderProspectCard(v) {
  ...
  s.append(el('div', { class: 'head' },
    ...
    el('div', {}, el('div', { class: 'hname' }, v.name.toUpperCase()),
      ...
      el('div', { class: 'hfacts' }, el('div', { 'data-tip': 'Where the league as a whole grades him, on the same scale as your estimate. The gap between the two is your edge or your blind spot' }, el('span', {}, 'Consensus'), el('b', {}, v.cons != null ? `${v.cons}${v.cons_rank ? ' · #' + v.cons_rank : ''}` : '—')), el('div', { 'data-tip': 'Your estimate minus the consensus. Positive: your room likes him more than the league, and he may reach you later than his talent; negative: the league likes him more than you do' }, el('span', {}, 'Gap'), el('b', { style: v.gap > 0 ? 'color:var(--ok)' : v.gap < 0 ? 'color:var(--danger)' : '' }, v.gap != null ? (v.gap > 0 ? '+' : '') + v.gap : '—')), el('div', { 'data-tip': 'The pick range where the consensus expects him to go. If you want him, plan on that range or earlier' }, el('span', {}, 'Projected'), el('b', {}, v.proj_range)), el('div', {}, el('span', {}, 'Your Board'), el('b', {}, v.dnd ? 'Do Not Draft' : v.on_board ? `#${v.on_board}` : 'Not placed')), el('div', {}, el('span', {}, 'Read'), el('b', { style: 'color:var(--ink-2)' }, `${v.confidence} · ${v.reads} look${v.reads === 1 ? '' : 's'}`)))),
    el('div', { class: 'ovrbig' }, el('b', { 'data-tip': "Your scouts' estimate of his overall today. It carries error: a first read can be a few points high or low, the combine tightens the physical part, a visit tightens the rest" }, v.mine), el('span', {}, 'Estimated Overall · your scouts'), el('div', { class: 'pot', 'data-tip': 'The range your scouts see him growing into. Wide means the room does not know yet; a visit narrows it' }, `Ceiling ${v.ceiling}${v.my_round ? ' · Your Grade ' + v.my_round : ''}`))));
...
function renderBoard(v) {
  ...
  const draw = () => {
    tbl.innerHTML = '';
    tbl.append(el('tr', {}, el('th', { class: 'n' }, '#'), el('th', {}, 'Prospect'), el('th', {}, 'Pos'), el('th', {}, 'School'), el('th', { class: 'n', 'data-tip': "Your scouts' estimate of his overall today. It carries error: a first read can be a few points high or low, the combine tightens the physical part, a visit tightens the rest" }, 'Estimated Overall'), el('th', { class: 'n', 'data-tip': 'The range your scouts see him growing into. Wide means the room does not know yet; a visit narrows it' }, 'Ceiling'), el('th', { class: 'n', 'data-tip': 'Where the league as a whole grades him, on the same scale as your estimate. The gap between the two is your edge or your blind spot' }, 'Consensus'), el('th', { class: 'n', 'data-tip': 'Your estimate minus the consensus. Positive: your room likes him more than the league, and he may reach you later than his talent; negative: the league likes him more than you do' }, 'Gap'), el('th', { class: 'n', 'data-tip': 'The pick range where the consensus expects him to go. If you want him, plan on that range or earlier' }, 'Proj.'), el('th', {}, 'Flags'), el('th', {}, '')));
...
function yourBoard(v, reload) {
  ...
  const t = el('table', { class: 'tbl' }); t.append(el('tr', {}, el('th', { class: 'n' }, '#'), el('th', {}, 'Player'), el('th', { class: 'n', 'data-tip': 'The pick range where the consensus expects him to go. If you want him, plan on that range or earlier' }, 'Proj.'), el('th', { class: 'n', 'data-tip': "Your scouts' estimate of his overall today. It carries error: a first read can be a few points high or low, the combine tightens the physical part, a visit tightens the rest" }, 'Est. Ovr'), el('th', {}, '')));
  let lastTier = null;
...
function renderSpring(v) {
  ...
  const vt = el('table', { class: 'tbl' }); vt.append(el('tr', {}, el('th', {}, 'Prospect'), el('th', {}, 'Pos'), el('th', { class: 'n', 'data-tip': "Your scouts' estimate of his overall today. It carries error: a first read can be a few points high or low, the combine tightens the physical part, a visit tightens the rest" }, 'Your Read'), el('th', { class: 'n', 'data-tip': 'The range your scouts see him growing into. Wide means the room does not know yet; a visit narrows it' }, 'Ceiling'), el('th', { class: 'n', 'data-tip': 'Where the league as a whole grades him, on the same scale as your estimate. The gap between the two is your edge or your blind spot' }, 'Consensus'), el('th', { class: 'n', 'data-tip': 'Your estimate minus the consensus. Positive: your room likes him more than the league, and he may reach you later than his talent; negative: the league likes him more than you do' }, 'Gap'), el('th', {}, 'Flags')));
...
function renderDraftDay(v) {
  ...
  const bt = el('table', { class: 'tbl' }); bt.append(el('tr', {}, el('th', { class: 'n' }, '#'), el('th', {}, 'Player'), el('th', { class: 'n', 'data-tip': 'The pick range where the consensus expects him to go. If you want him, plan on that range or earlier' }, 'Proj.'), el('th', { class: 'n', 'data-tip': "Your scouts' estimate of his overall today. It carries error: a first read can be a few points high or low, the combine tightens the physical part, a visit tightens the rest" }, 'Est. Ovr')));
